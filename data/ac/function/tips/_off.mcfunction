### 关闭提示系统
## 日志: 开始关闭tips系统（重要事件，保留在debugger队伍）
...
## 日志: 已重置trigger分数（移至high_freq_log队伍）
 tellraw @a[team=high_freq_log] [{text:"[ACPack_HIGH_FREQ] ",color:"blue"},{text:"tips/_off.mcfunction - 已设置trigger.int=3并重置trigger",color:"green"}]
