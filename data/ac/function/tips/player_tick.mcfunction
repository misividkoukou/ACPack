### 玩家提示系统控制函数
## 日志: player_tick函数开始执行
 tellraw @a[team=debugger] [{text:"[ACPack_DEBUG] ",color:"dark_purple"},{text:"tips/player_tick.mcfunction - 开始执行玩家每刻检测",color:"white"}]
...
## 日志: 检查是否首次进入
 tellraw @a[team=debugger] [{text:"[ACPack_DEBUG] ",color:"dark_purple"},{text:"tips/player_tick.mcfunction - 检查玩家trigger.int状态: ",color:"white"},{score:{name:"@s",objective:"trigger.int"},color:"yellow"}]
execute unless score @s trigger.int matches 1.. run scoreboard players set @s trigger 1
## 日志: 首次进入处理完成
 tellraw @a[team=debugger] [{text:"[ACPack_DEBUG] ",color:"dark_purple"},{text:"tips/player_tick.mcfunction - 首次进入处理完成",color:"green"}]

#### 触发器状态处理
## 运行
#### 这里加范围是防止玩家滥用其他数字
## 日志: 检查trigger触发值
 tellraw @a[team=debugger] [{text:"[ACPack_DEBUG] ",color:"dark_purple"},{text:"tips/player_tick.mcfunction - 当前trigger触发值: ",color:"white"},{score:{name:"@s",objective:"trigger"},color:"yellow"}]
